// SmallOLED Web Flasher — BETA channel client logic.
// Same engine as the main flasher (docs/flasher.js) but self-contained under
// docs/beta/: reads its version + bins from beta/firmware/ so it can ship a
// pre-release build without touching the stable channel. Used to hand testers
// a specific fix (e.g. the ESP32-C3 clone WiFi cap, issue #61) before it goes
// stable. Three display variants share two firmware images: SSD1306 (0.96") and
// SSD1309 (2.42") use the same binary; SH1106 (1.3") and CH1116 (1.54") differ.
...
async function loadVersion() {
  const r = await fetch('firmware/VERSION', { cache: 'no-cache' });
  if (!r.ok) throw new Error(`firmware/VERSION returned HTTP ${r.status}`);
...
function buildManifest(boardId, version) {
  ...
  const binUrl = new URL(
    `firmware/SmallOLED-${board.firmware}-${version}-Full.bin`,
    location.href,
  ).href;
  return {
    name: 'SmallOLED PC-Monitor (beta)',
    version,
    ...
    // Tools show its in-browser "Configure WiFi" dialog. The WiFiManager AP
    // portal stays up in parallel as a fallback.
...
function checkBrowserSupport() {
  if (!('serial' in navigator)) {
    const callout = document.getElementById('browser-callout');
    if (callout) callout.classList.add('show');
  }
}
